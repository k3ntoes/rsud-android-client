...
> ⚠️ **Endpoint ini BELUM ada di backend** — dependensi untuk fitur re-upload manual Android (ADR-0016). `POST /api/upload` biasa selalu menghasilkan UUID baru yang TIDAK terhubung ke inspeksi; endpoint inilah yang menyambungkan file baru ke referensi photo yang sudah ada.
...
| **PUT** | **`/api/inspections/{id}/photos/{photoId}`** | **Jarang (re-upload manual)** | **Bearer** | **Replace foto terkirim — dependensi ADR-0016** |
...
| **P3** | Replace Photo (`PUT /api/inspections/{id}/photos/{photoId}`) | Sedang | Dependensi fitur re-upload manual Android (ADR-0016); hapus file lama + regenerate thumbnail |
